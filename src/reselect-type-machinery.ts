...
import { QueryContext } from "./redux";
/**
 *
 * FORK
 *
 * Fork of reselect types that drops params.
 *
 *
 */
 ...
 */
export declare type Selector<A = unknown, E = unknown, S = never> = (
  state: S
) => Q.Query<A, E, QueryContext<S>>;
 ...
 */
export declare type OutputSelector<S extends SelectorArray, A, E> = Selector<
  A,
  E,
  GetStateFromSelectors<S>
>;
...
export declare type SelectorResultArray<Selectors extends SelectorArray> =
  ExtractReturnType<Selectors>;
/** Determines the combined single "State" type (first arg) from all input selectors */
export declare type GetStateFromSelectors<S extends SelectorArray> =
  MergeParameters<S>[0];
 ...
 */
export declare type MergeParameters<
...
export declare type Longest<L extends List, L1 extends List> = L extends unknown
  ? L1 extends unknown
    ? {
        0: L1;
        1: L;
 ...
 */
export declare type LongestArray<S extends readonly any[][]> =
  IsTuple<S> extends "0"
    ? S[0]
    : S extends [any[], any[]]
    ? Longest<S[0], S[1]>
...
export declare type IntersectAll<T extends any[]> = IsTuple<T> extends "0"
  ? T[0]
  : _IntersectAll<T>;
declare type IfJustNullish<T, True, False> = [T] extends [undefined | null]
  ? True
  : False;
 ...
 */
declare type _IntersectAll<T, R = unknown> = T extends [
  infer First,
  ...infer Rest
]
...
declare type LastOf<T> = UnionToIntersection<
  T extends any ? () => T : never
> extends () => infer R
  ? R
  : never;
declare type TuplifyUnion<
  T,
  L = LastOf<T>,
 ...
 */
export declare type ObjValueTuple<
  T,
 ...
 */
declare type Transpose<T> = T[Extract<
  keyof T,
...
export declare type DropFirst<T extends unknown[]> = T extends [
  unknown,
  ...infer U
]
  ? U
...
export declare type Expand<T> = T extends (...args: infer A) => infer R
  ...
  : never;
export declare type ExpandRecursively<T> = T extends (
  ...args: infer A
) => infer R
...
export declare type If2<B extends Boolean2, Then, Else = never> = B extends 1
  ? Then
  : Else;
